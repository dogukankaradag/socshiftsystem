// Paylaşılan liste/upload/düzenle paneli — hem "Nöbetçi Listesi" (l2/mssp)
// hem de "Dağıtıcı Listesi" (distributor/lunch) sayfaları bu bileşeni
// `teams` prop'u ile özelleştirerek kullanır. UI, dark-mode dahil
// proje standart class'larını (card, input, btn-*) kullanır.
import { FormEvent, useEffect, useMemo, useRef, useState } from 'react';
import {
  api,
  ROSTER_TEAM_LABEL,
  ROSTER_TEAMS_WITH_SHIFT_LABEL,
  RosterEntry,
  RosterTeam,
  RosterUploadResult,
} from '../api/client';
...
export interface RosterListPageProps {
  /** Bu sayfada gösterilecek takımlar (sekme olarak). En az 1. */
  teams: RosterTeam[];
  /** Sayfa başlığı (h1). */
  pageTitle: string;
  /** Sayfa alt yazısı (açıklama). */
  pageSubtitle: string;
  /** Etiket "Dosya saklanmaz, ..." metni override edilebilsin diye. */
  uploadHint?: string;
}
...
export default function RosterListPage({
  ...
  const showShiftCol = ROSTER_TEAMS_WITH_SHIFT_LABEL.has(team);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400">{pageSubtitle}</p>
        </div>
        {canEdit && (
      ...
      <div className="card space-y-3">
          ...
          {teams.map((t) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md border text-sm font-medium ${
                team === t
                  ? 'border-brand-600 bg-brand-50 text-brand-700 dark:bg-slate-700 dark:border-brand-400 dark:text-brand-400'
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-slate-200 mt-6">
            <input
              type="checkbox"
              checked={activeOnly}
        ...
        {canEdit && (
          <form
            ...
            className="flex flex-wrap items-end gap-3 bg-gray-50 dark:bg-slate-700/40 border border-dashed border-gray-300 dark:border-slate-600 rounded-md p-3"
          >
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">
                Seçili ekip: <b>{ROSTER_TEAM_LABEL[team]}</b>.{' '}
                {uploadHint ?? 'Dosya saklanmaz, yalnızca satırlar veritabanına yazılır.'}
              </p>
            </div>
        ...
        {msg && <div className="text-sm text-gray-700 dark:text-slate-200">{msg}</div>}
        {warnings.length > 0 && (
          <details className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded p-2 dark:bg-amber-900/30 dark:border-amber-800/50 dark:text-amber-200">
            <summary className="cursor-pointer">Uyarılar ({warnings.length})</summary>
      ...
      <div className="card p-0 overflow-hidden">
        {loading ? (
          <div className="p-6 text-gray-500 dark:text-slate-400">Yükleniyor…</div>
        ) : items.length === 0 ? (
          ...
          <div>
            {[...groupedByBatch.entries()].map(([batch, rows]) => (
              <div key={batch}>
                <div className="flex items-center justify-between px-4 py-2 bg-gray-50 dark:bg-slate-700/40 border-b border-gray-200 dark:border-slate-700 text-xs text-gray-600 dark:text-slate-300">
                  <span>
                    {batch === '_manual'
                      ? 'Elle eklenen'
                      ...
                      <tr key={r.id}>
                        ...
                        {canEdit && (
                          <td className="px-4 py-2 text-right whitespace-nowrap space-x-2">
                            <button
                              className="text-xs text-gray-700 hover:text-brand-700 dark:text-slate-200 dark:hover:text-brand-400"
  ...
  return (
    <form onSubmit={submit} className="card space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        {showShift && (
          <div>
            <label className="label">Vardiya / Etiket</label>
            <input
              className="input"
              maxLength={16}
              value={shiftLabel}
              onChange={(e) => setShiftLabel(e.target.value)}
              placeholder={team === 'mssp' ? 'A' : 'Öğle 12:00'}
            />
          </div>
        )}
...
function EditRosterModal({
  ...
  const showShift = ROSTER_TEAMS_WITH_SHIFT_LABEL.has(team);
